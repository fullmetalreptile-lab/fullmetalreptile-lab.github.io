...
function renderSlide(index) {
  ...
  if (slide.type === 'youtube') {
    ...
    waitTwitch(function() {
      var player = new Twitch.Player(id, { channel: 'fullmetalreptile', width: '100%', height: '100%', autoplay: true, muted: true });
    ...
    viewerContent.appendChild(div);
    waitTwitch(function() {
      var player = new Twitch.Player(id, { video: slide.id, width: '100%', height: '100%', autoplay: true, muted: true });
      player.addEventListener(Twitch.Player.READY, function() {
        player.setMuted(true);
        player.play();
      });
